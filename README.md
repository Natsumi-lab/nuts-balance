...
Next.js App Routerを使うことで、画面の処理（Client）とデータ処理（Server）  
を分けて実装しやすいと考えました。  
またServer Actionsを利用することで、APIを作成しなくてもサーバー側で安全に  
データ更新処理ができる点も魅力でした。
...
### 3. RLSによるデータ保護
Supabase のRow Level Securityを利用し、  
ログインユーザーは自分のデータのみ取得可能にしました。 
...
Vitestを使用し、ドメインロジックのユニットテストを行っています。
